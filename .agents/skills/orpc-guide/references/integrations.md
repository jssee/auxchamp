...
const query = useQuery(computed(() => orpc.planet.find.queryOptions({ input: { id: id.value } })));

// Solid - function options
const query = useQuery(() => orpc.planet.find.queryOptions({ input: { id: id() } }));
```
...
```ts
const { data } = useSWRSubscription(orpc.live.key({ input: { id: 3 } }), orpc.live.subscriber());
```
...
const complete = os.input(z.object({ content: z.string() })).handler(async function* ({ input }) {
