...
```ts
const onMessage = os.input(z.object({ roomId: z.string() })).handler(async ({ input, context }) => {
  ...
  .handler(async ({ input, context }) => {
    ...
    for (const ws of websockets) {
      ...
      if (attachment.roomId === input.roomId) {
        // Send regular event
        ws.send(
          encodeHibernationRPCEvent(attachment.id, {
...
const joinRoom = os.input(z.object({ roomId: z.string() })).handler(async ({ input, context }) => {
  ...
  return new HibernationEventIterator<{
    ...
    for (const ws of websockets) {
      ...
      if (att.roomId === input.roomId && att.userId !== userId) {
        ws.send(
          encodeHibernationRPCEvent(att.id, {
            type: "join",
            userId,
          }),
        );
