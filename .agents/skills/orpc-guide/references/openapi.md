...
const spec = await generator.generate(router, {
  ...
  servers: [{ url: "https://api.example.com/v1" }, { url: "http://localhost:3000/api" }],
});
...
interface OpenAPIHandlerOptions {
  ...
  filter?: (info: { contract: ContractProcedure; path: string[] }) => boolean;
...
```ts
// Simple parameter
os.route({ path: "/users/{id}" }).input(z.object({ id: z.string() }));

// Multiple parameters
os.route({ path: "/users/{userId}/posts/{postId}" }).input(
  z.object({
    userId: z.string(),
    postId: z.string(),
  }),
...
os.route({ path: "/files/{+path}" }).input(z.object({ path: z.string() }));
```
...
// JSON Schema customization
const Schema = oz.openapi(z.object({ name: z.string() }), {
  examples: [{ name: "Earth" }],
});
```
...
const findUser = os.route({ path: "/users/{id}" }).handler(async ({ input }) => {
...
// Version in header
const versionedProcedure = os.route({ path: "/planets" }).handler(({ context }) => {
