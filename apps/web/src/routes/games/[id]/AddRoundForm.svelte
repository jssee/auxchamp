...
<form class="mt-3" {...addRound}>
	...
	<Field.Group class="gap-3">
		<div class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
			<Field.Field
				class="min-w-0"
				data-invalid={addRound.fields.theme.issues()?.length ? 'true' : undefined}
			>
			...
			<Button type="submit" class="w-full self-start sm:mt-6 sm:w-auto" disabled={addRound.pending > 0}>
				{addRound.pending > 0 ? 'Adding...' : 'Add Round'}
			</Button>
		</div>

		<Field.Field data-invalid={addRound.fields.description.issues()?.length ? 'true' : undefined}>
			...
			/>
			<Field.Description>Optional context players should see for this round.</Field.Description>
