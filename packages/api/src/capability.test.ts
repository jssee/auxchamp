...
describe("getAllowedActions", () => {
  ...
  test("invited player can only accept invite", () => {
    ...
    expect(actions.includes("accept_invite")).toBe(true);
  ...
  describe("draft game", () => {
    test("creator gets draft actions", () => {
      ...
      expect(actions.includes("edit_game")).toBe(true);
      expect(actions.includes("edit_future_round")).toBe(true);
      expect(actions.includes("invite_player")).toBe(true);
    });
    ...
    test("creator can start with enough players and rounds", () => {
      ...
      expect(actions.includes("start_game")).toBe(true);
    });
    ...
    test("creator cannot start with too few players", () => {
      ...
      expect(actions.includes("start_game")).toBe(false);
    });
    ...
    test("non-creator player gets no draft actions", () => {
      ...
      expect(actions.includes("edit_game")).toBe(false);
      expect(actions.includes("invite_player")).toBe(false);
      expect(actions.includes("start_game")).toBe(false);
    });
  });
  ...
  describe("active game — submitting phase", () => {
    ...
    test("creator can transition during submitting phase", () => {
      ...
      expect(actions.includes("transition_round")).toBe(true);
    });
  ...
  describe("active game — voting phase", () => {
    test("active player can vote", () => {
      ...
      expect(actions.includes("cast_ballot")).toBe(true);
      expect(actions.includes("leave_game")).toBe(true);
    });
  ...
  describe("completed game", () => {
    test("no actions available", () => {
      ...
      expect(actions.includes("cast_ballot")).toBe(false);
      expect(actions.includes("transition_round")).toBe(false);
      expect(actions.includes("leave_game")).toBe(false);
    });
  });
});
